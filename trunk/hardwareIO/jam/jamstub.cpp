...
int jam_jtag_io(int tms, int tdi, int read_tdo)
{
	...
	m_pSC->PCIO_Write(address, data);
    
    if (read_tdo)
	{
		m_pSC->PCIO_Read(address, &data);
		tdo = (data & 1);
	}
...
int jam(char* filename, char* action)
{
	...
	{
		if (!initialize_jtag_hardware(NULL))
			return DRIVER_ERROR;
	}
...
int initialize_jtag_hardware(TCPcio* m_pHardware)
{
	if (!jtag_hardware_initialized)
	{
		if (m_pHardware != NULL)
		{
			m_pSC = m_pHardware;//if (!pciioOpen())
				//return FALSE;
...
{
	m_pSC->PCIO_SetDigital();
	m_pSC = NULL;//pciioClose();
...
void delay_loop(int count)
{
	...
	while (count != 0)
	{
		m_pSC->PCIO_Write(address, data);//pciioWrite(address, &data);
		count--;
	}
}
